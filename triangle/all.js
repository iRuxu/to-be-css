/*1414566439,,JIT Construction: v1472508,zh_HK*/
...
try {
	window.FB || (function(window) {
		...
		__d("JSSDKXDConfig", [], {
			...
			"XdBundleUrl": "\/connect\/xd_arbiter\/ehazDpFPEnK.js?version=41",
			"Flash": {
		...
		__d("ApiClient", ["ArgumentError", "Assert", "CORSRequest", "FlashRequest", "flattenObject", "JSONPRequest", "Log", "ObservableMixin", "sprintf", "sdk.URI", "UrlMap", "ApiClientConfig", "invariant"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s) {
			...
			function ga() {
				...
				var oa = fa(la),
					...
					ra = oa.params,
					sa = ra.method,
					ta = pa.getProtocol() && pa.getDomain() ? pa.setQueryData({}).toString() : q.resolve('graph') + pa.getPath();
				da(ta, sa == 'get' ? 'get' : 'post', ra, ES(ea, 'bind', true, null, qa, pa.getPath(), sa, ra));
			}
			...
			var w = n.extend({
				...
				x = {
					...
					_createMobileLoader: function() {
						var y = o.nativeApp() ? '' : ('<table>' + '  <tbody>' + '    <tr>' + '      <td class="header_left">' + '        <label class="touchable_button">' + '          <input type="submit" value="' + k.tx._("\u53d6\u6d88") + '"' + '            id="fb_dialog_loader_close"/>' + '        </label>' + '      </td>' + '      <td class="header_center">' + '        <div>' + k.tx._("\u8f09\u5165\u4e2d......") + '</div>' + '      </td>' + '      <td class="header_right">' + '      </td>' + '    </tr>' + '  </tbody>' + '</table>');
						return x.create({
	...
	new Image().src = "http:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m=' + encodeURIComponent('{"error":"LOAD", "extra": {"name":"' + e.name + '","line":"' + (e.lineNumber || e.line) + '","script":"' + (e.fileName || e.sourceURL || e.script) + '","stack":"' + (e.stackTrace || e.stack) + '","revision":"1472508","message":"' + e.message + '"}}');
}